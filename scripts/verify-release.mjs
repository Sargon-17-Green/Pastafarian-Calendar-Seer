...
async function bytes(url, options) {
  return new Uint8Array(await (await get(url, options)).arrayBuffer());
}
...
if (!allowIncomplete) {
  const requiredAssets = [
    manifestAsset,
    sumsAsset,
    release.assets.find((asset) => asset.name === npmSbomName),
    release.assets.find((asset) => asset.name === amd64SbomName),
    release.assets.find((asset) => asset.name === arm64SbomName),
  ];
  if (requiredAssets.some((asset) => !asset)) {
    throw new Error('hardened release asset set is incomplete');
  }
}
...
if (sumsAsset) {
  ...
  for (const name of requiredNames) {
    ...
    if (actual !== expected) {
      throw new Error(`release asset SHA-256 mismatch for ${name}: ${actual} != ${expected}`);
    }
  }
...
if (manifestAsset) {
  const manifest = JSON.parse(
    new TextDecoder().decode(await bytes(manifestAsset.browser_download_url)),
  );
  if (manifest.version !== version || manifest.tag !== tag || manifest.commit !== commit) {
    throw new Error('release manifest identity mismatch');
  }
  if (manifest.githubRelease?.tarballSha256 !== githubSha256) {
  ...
  for (const platform of ['linux/amd64', 'linux/arm64']) {
    ...
    if (!allowIncomplete && (!recorded || !actual)) {
      throw new Error(`release manifest platform missing: ${platform}`);
    }
    if (recorded && actual && recorded !== actual) {
      throw new Error(`release manifest ${platform} digest mismatch`);
    }
  }

  if (!allowIncomplete) {
    if (manifest.npm?.sbom !== npmSbomName) throw new Error('release manifest npm SBOM name mismatch');
    if (manifest.container?.sboms?.['linux/amd64'] !== amd64SbomName) {
      throw new Error('release manifest amd64 SBOM name mismatch');
    }
    if (manifest.container?.sboms?.['linux/arm64'] !== arm64SbomName) {
      throw new Error('release manifest arm64 SBOM name mismatch');
    }
  }
...
if (!allowIncomplete && (status.npm?.missing || status.container?.missing || status.manifest?.missing)) {
  process.exit(1);
}
