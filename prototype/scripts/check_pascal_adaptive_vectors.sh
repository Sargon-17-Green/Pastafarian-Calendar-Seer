...
(
  ...
  for t in 2461247 2461290 2462913 2464579 -12829630 6788193; do
    a=$(./pascal_adaptive_base 2461290 "$t" "$THREADS" "$SB" "$REPLAY_THREADS" | head -n1)
    b=$(./pascal_adaptive_candidate 2461290 "$t" "$THREADS" "$SB" "$REPLAY_THREADS" | head -n1)
    if [[ "$a" != "$b" ]]; then echo "target=$t equal=NO"; echo "base: $a"; echo "cand: $b"; exit 1; fi
    echo "target=$t equal=YES"
  done
)
echo "Adaptive Pascal-ladder full-vector validation: PASS"
