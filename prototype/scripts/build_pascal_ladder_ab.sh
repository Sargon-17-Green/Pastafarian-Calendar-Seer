...
GEN="$BUILD/pastafarian_cold_bench_avx2_split_pascal_ladder.cpp"
python3 - "$BASE" "$GEN" <<'PY'
...
Path(sys.argv[2]).write_text(s.replace(needle,'#include "rns_micro8_avx2_32x8_pascal_ladder.cpp"'))
PY
"$CXX" "${COMMON[@]}" "$BASE" -lgmpxx -lgmp -o "$BUILD/pascal_ladder_base"
"$CXX" "${COMMON[@]}" "$GEN" -lgmpxx -lgmp -o "$BUILD/pascal_ladder_candidate"
"$CXX" "${COMMON[@]}" "$RESEARCH_SRC/rns_micro8_avx2_32x8_pascal_ladder.cpp" -lgmpxx -lgmp -o "$BUILD/pascal_ladder_selftest"
ln -sf ../data/gates_u16.bin "$BUILD/gates_u16.bin"
echo "Built AVX2 Pascal-ladder A/B binaries."
