...
mkdir -p "$BUILD"
CXX=${CXX:-g++}
FLAGS=(-O3 -std=c++20 -march=x86-64-v3 -fopenmp -I"$SRC" -I"$RESEARCH_SRC")
...
"$CXX" "${FLAGS[@]}" -DFRACDOUBLE_DIAGNOSTIC "$RESEARCH_SRC/rns_micro8_avx2_32x8_fracdouble.cpp" "${LIBS[@]}" -o "$BUILD/fracdouble_selftest"
