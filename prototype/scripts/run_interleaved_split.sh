...
run_one() {
  local backend="$1" name="$2" calc="$3" target="$4" replay="$5" rep="$6" phase="$7" exe
...
for row in 'same-end 2461290 2463007' 'far-past-3540y 2461290 -12829630'; do
  read -r name calc target <<<"$row"
  for replay in 1 2 4 2 1; do run_one avx2 "$name" "$calc" "$target" "$replay" 0 sweep; done
done
