...
BUILD=ROOT/'build'; RESULTS=ROOT/'results'; RESULTS.mkdir(parents=True,exist_ok=True)
...
cases=[('same-end',2461290,2463007),('far-past-3540y',2461290,-12829630),('forward-994y',2461290,6700000)]
rx=re.compile(r'(\w+)=([0-9.]+)'); rows=[]
for rep in range(1,reps+1):
    jobs=[(b,c) for c in cases for b in backends]; random.Random(0x5EED0000+rep).shuffle(jobs)
    for b,(case,calc,target) in jobs:
        ...
        summary,timing=lines[0],lines[-1]; vals={k:float(v) for k,v in rx.findall(timing)}
        ...
        row={'rep':rep,'backend':b,'case':case,'calc':calc,'target':target,'external_wall_s':float(tm.group(1)),'max_rss_kb':int(tm.group(2)),
             'all_ms':vals['all'],'walk_ms':vals['walk'],'rns_ctor_ms':vals['rns_ctor'],'rns_count_ms':vals['rns_count'],'prefix_total_ms':vals['prefix_total'],
             'replay_ms':vals['replay'],'reset_ms':vals['reset'],'ucrt_ms':vals['ucrt'],'cert':int(vals['cert']),'splits':int(vals['splits']),'micro':int(vals['micro']),'summary':summary}
        rows.append(row); print(f"rep={rep} backend={b} case={case} all={row['all_ms']:.3f} replay={row['replay_ms']:.3f} rss_kb={row['max_rss_kb']}",flush=True)
        if cooldown: time.sleep(cooldown)
with (RESULTS/'replay-cache-ab.csv').open('w',newline='') as f:
...
for case,_,_ in cases:
    ...
    for b in backends:
        ...
        med={k:statistics.median(r[k] for r in rr) for k in ('all_ms','replay_ms','prefix_total_ms','rns_ctor_ms','rns_count_ms','max_rss_kb')}; meds[b]=med
    ...
    out.append('')
summary='\n'.join(out); (RESULTS/'replay-cache-summary.txt').write_text(summary+'\n'); print('\n'+summary)
