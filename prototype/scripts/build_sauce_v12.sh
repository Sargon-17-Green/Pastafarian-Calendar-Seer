...
"$CXX" "${COMMON[@]}" "$RESEARCH_SRC/pastafarian_cold_bench_avx2_split_v3_reference.cpp" -lgmpxx -lgmp -o "$ROOT/build/seer_avx2_v3_split"
"$CXX" "${COMMON[@]}" "$RESEARCH_SRC/pastafarian_cold_bench_avx2_split_v12.cpp" -lgmpxx -lgmp -o "$ROOT/build/seer_avx2_v12_split"
