...
static FSauce fast_sauce(int64_t cj,int64_t tj,const FStones&S,FSauceTrace* trace=nullptr){
    ...
    for(int drop=1;drop<=46;drop++){auto&s=S[drop-1];U128 p=seqv(drop-1),p3=seqv(drop-3),p7=seqv(drop-7),v=fsum({cmul(s[0],calc),cmul(s[1],target),cmul(s[2],dist),cmul(s[3],sum),fmul_small(s[4],(uint64_t)dir),p,fmul_small(p3,3),fmul_small(p7,5),U128(drop)});U128 tt=fsum({p,p3,p7});U128 p32=fadd(p3,p3),p34=fadd(p32,p32),p36=fadd(p34,p32);U128 p72=fadd(p7,p7),p74=fadd(p72,p72),p76=fadd(p74,p72),p78=fadd(p74,p74),p710=fadd(p78,p72);for(int g=0;g<11;g++){const auto&qg=FVG[g];int d2=qg[2]-qg[1],d3=qg[3]-qg[1];U128 a2=d2==2?p32:(d2==4?p34:p36);U128 a3=d3==6?p76:(d3==8?p78:p710);U128 gc=fsum({fmul_small(tt,qg[1]),a2,a3,s[qg[4]]});v=fadd(fmul(v,fadd(v,U128(qg[0]))),gc);}seq[drop]=v;auto ord=fperm720(frank720(v));last=ord;auto old=b;std::array<U128,6>direct{};for(int pl=0;pl<3;pl++){int bi=ord[pl]-1;direct[bi]=fsum({fsq(v),fmul(s[FDST[pl]],old[bi]),U128(FDM[pl]*drop)});}std::array<U128,6>next{};for(int pl=0;pl<6;pl++){int bi=ord[pl]-1,pi=ord[(pl+5)%6]-1,ni=ord[(pl+1)%6]-1;U128 u=fsum({old[bi],fmul_small(old[pi],2),fmul_small(old[ni],3),direct[bi],v,s[FPST[pl]]});next[bi]=fsum({fsq(u),fmul_small(fmul(old[pi],old[ni]),5),U128(drop*(pl+1))});}b=next;}
    if(trace)trace->afterDrop46=b;
    for(int round=1;round<=12;round++){auto old=b;U128 raw=0;for(U128 x:old)raw=fadd(raw,x);U128 savedSumR=fadd(raw,U128(149*round));auto ord=fperm720(frank720(savedSumR));std::array<U128,6>next{};for(int pl=0;pl<6;pl++){int bi=ord[pl]-1,pi=ord[(pl+5)%6]-1,ni=ord[(pl+1)%6]-1;U128 u=fsum({old[bi],fmul_small(old[pi],3),fmul_small(old[ni],5),savedSumR,U128(round+(pl+1)*(pl+1))});next[bi]=fadd(fsq(u),fmul_small(fmul(old[pi],old[ni]),7));}b=next;if(trace)trace->postStirs[round-1]=b;}
    return{b,last};
}
