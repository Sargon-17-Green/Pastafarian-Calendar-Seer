...
[[maybe_unused]] static BI yb_fast_choose_bi(const FSauce&so,int bowl,uint64_t seal,const BI&n,int*widthOut=nullptr){
    if(n<=0)throw std::runtime_error("n<=0");
    auto d=fast_desc(so,bowl,seal);BI MM=yb_fu_to_bi(F127_M);
...
[[maybe_unused]] static mpz_class yb_fast_choose_mpz(const FSauce&so,int bowl,uint64_t seal,const mpz_class&n,int*widthOut=nullptr){
    if(n<=0)throw std::runtime_error("n<=0");
    auto d=fast_desc(so,bowl,seal);mpz_class MM=yb_fu_to_mpz(F127_M);
