...
static int conservative_npr(const std::vector<int>&len){
    int total=std::accumulate(len.begin(),len.end(),0);long double ln=lgammal((long double)total+1);for(int x:len)ln-=lgammal((long double)x+1);long double bits=ln/logl(2.0L);int k=(int)ceill((bits+32.0L)/51.9L)+2;return std::clamp(k,8,610);
}
