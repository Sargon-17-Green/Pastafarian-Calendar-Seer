...
#ifdef FRACDOUBLE_DIAGNOSTIC
static uint64_t FRACDOUBLE_FAST=0,FRACDOUBLE_FALLBACK=0;
#define FRACDOUBLE_HIT_FAST() (++FRACDOUBLE_FAST)
#define FRACDOUBLE_HIT_FALLBACK() (++FRACDOUBLE_FALLBACK)
#else
#define FRACDOUBLE_HIT_FAST() ((void)0)
#define FRACDOUBLE_HIT_FALLBACK() ((void)0)
#endif
...
static FracTriple frac_state_vector(const RnsEngine&e,const FastState&s,const std::vector<uint64_t>&cf,int maxlayers=12){
    ...
    for(int layer=0;layer<maxlayers;layer++){
        uint64_t sumq[3]{0,0,0};double sumd[3]{0,0,0};
        for(int pk=0;pk<packs;pk++){
            ...
            for(int l=0;l<avail;l++){sumq[0]+=xu[l];sumq[1]+=xv[l];sumq[2]+=xs[l];double den=(double)e.primes[base+l];sumd[0]+=(double)yu[l]/den;sumd[1]+=(double)yv[l]/den;sumd[2]+=(double)ys[l]/den;}
        }
        uint64_t carryv[3]{};
        constexpr double SAFE=2.0e-9;
        for(int a=0;a<3;a++){
            double fl=std::floor(sumd[a]),fr=sumd[a]-fl;
            if(fr>SAFE && (1.0-fr)>SAFE){FRACDOUBLE_HIT_FAST();carryv[a]=(uint64_t)fl;continue;}
            FRACDOUBLE_HIT_FALLBACK();
            long double sl=0,cr=0;
            for(int pk=0;pk<packs;pk++){int base=pk*8,avail=std::min(8,k-base);alignas(64)uint64_t yy[8];if(a==0)vstore(yy,nu[pk]);else if(a==1)vstore(yy,nv[pk]);else vstore(yy,ns[pk]);for(int l=0;l<avail;l++){long double term=(long double)yy[l]*e.invp[base+l];long double z=term-cr,t=sl+z;cr=(t-sl)-z;sl=t;}}
            carryv[a]=(uint64_t)floorl(sl+1e-18L);
        }
        FracAdaptive* rr[3]={&out.u,&out.v,&out.s};bool all=true;
        for(int a=0;a<3;a++){if(done[a])continue;uint64_t carry=carryv[a];uint64_t dig=(sumq[a]+carry)&MASK52;rr[a]->layers=layer+1;if(dig){if(!seen[a])rr[a]->firstnz=layer+1;seen[a]=true;sig[a]++;}else if(seen[a])sig[a]++;val[a]+=scalbnl((long double)dig,-32*(layer+1));if(seen[a]&&sig[a]>=3)done[a]=true;if(!done[a])all=false;}
...
#ifdef FRACDOUBLE_DIAGNOSTIC
    std::cerr<<"fracdouble_fast="<<FRACDOUBLE_FAST<<" fracdouble_fallback="<<FRACDOUBLE_FALLBACK<<"\n";
#endif
}
