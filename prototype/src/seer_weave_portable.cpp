// Portable scalar RNS backend for the Seer.
// Derived lane-for-lane from rns_micro8.cpp; the IFMA baseline remains unchanged.
...
struct V8{std::array<uint64_t,8> v{};};
static inline V8 vzero(){return {};}
static inline V8 vsmall(uint64_t x){V8 r;for(auto &z:r.v)z=x;return r;}
static inline V8 vload(const uint64_t* p){V8 r;for(int i=0;i<8;i++)r.v[i]=p[i];return r;}
static inline void vstore(uint64_t* p,const V8&x){for(int i=0;i<8;i++)p[i]=x.v[i];}
static inline V8 vmaskload(int avail,const uint64_t* p){V8 r;for(int i=0;i<avail;i++)r.v[i]=p[i];return r;}

struct VMod{
    static inline V8 mul(const V8&a,const V8&b,const V8&p,const V8&){V8 r;for(int i=0;i<8;i++)r.v[i]=mulmod52(a.v[i],b.v[i],p.v[i]);return r;}
    static inline V8 mul_small(const V8&a,uint64_t b,const V8&p,const V8&){V8 r;for(int i=0;i<8;i++)r.v[i]=mulmod52(a.v[i],b,p.v[i]);return r;}
    static inline V8 add(const V8&a,const V8&b,const V8&p){V8 r;for(int i=0;i<8;i++)r.v[i]=addmod52(a.v[i],b.v[i],p.v[i]);return r;}
    static inline V8 sub(const V8&a,const V8&b,const V8&p){V8 r;for(int i=0;i<8;i++)r.v[i]=submod52(a.v[i],b.v[i],p.v[i]);return r;}
};
...
struct Predictor{
    ...
    int step(){auto ps=probs();if(!ok||ps.empty()){ok=false;return -1;}int n=static_cast<int>(ps.size());std::vector<long double> pre(n+1),suf(n+1);for(int i=0;i<n;i++)pre[i+1]=pre[i]+ps[i].p;for(int i=n-1;i>=0;i--)suf[i]=suf[i+1]+ps[i].p;bool left=u<=v;int idx=-1;if(left){for(int i=0;i<n;i++)if(u<pre[i+1]){idx=i;break;}}else{for(int i=n-1;i>=0;i--)if(v<suf[i]){idx=i;break;}}if(idx<0){idx=left?n-1:0;forced++;}long double p=ps[idx].p;if(!(p>0)){ok=false;return -1;}long double nu=(u-pre[idx])/p,nv=(v-suf[idx+1])/p;if(!std::isfinite(nu)||!std::isfinite(nv)){ok=false;return -1;}u=nu;v=nv;auto b=ps[idx];int month=b.month;if(b.open){st.high=month;st.rem[month]--;st.R+=tab->len[month]-1;if(st.low>month-1)st.low=month;}else{st.rem[month]--;st.R--;if(st.rem[month]==0)st.low++;advance_d();if(!ok)return -1;}st.pos++;return month;}
};

struct StepMeta{int low,high,R,d,chosen;bool open;std::array<int,MAXM> cum{};};
static StepMeta make_meta_and_apply(StructState& s,const std::vector<int>&len,int chosen){
...
static inline V8 frac_digit_step(const V8&n,const PackConst&q,V8 &qsumv){
    ...
    for(int i=0;i<8;i++){
        ...
        __uint128_t cn=(__uint128_t)cc*n.v[i];
        uint64_t q0=(uint64_t)(cn>>52),r0=(uint64_t)cn&MASK52;
        uint64_t yy=r0+q0*cc,ex=q0;
        if(yy>=p){yy-=p;ex++;}
        y.v[i]=yy;qsumv.v[i]=n.v[i]+ex;
    }
    return y;
}
...
std::vector<int> weave_month_prefix(
    ...
    long double ln=lgammal((long double)total+1),denomLn=0;
    for(int x:monthLengths) denomLn+=lgammal((long double)x+1);
    long double bits=(ln-denomLn)/logl(2.0L);
    ...
    FastState fs; fs.st=initial_struct(monthLengths); fs.pack=eng.initPacks;
    fs.k=initk; fs.coeff=coeff; fs.rank=rank; fs.total=N; set_rank_resid(eng,fs);
    Unknown u{eng,ap,gtDummy,dummyGold,pool,superblock};
    u.run(fs,stopPositions);
    if((int)u.out.size()<stopPositions) throw std::runtime_error("weave prefix shorter than requested segment");
    return u.out;
}
} // namespace seer_native::detail
