...
static constexpr uint64_t B52=1ULL<<52;
...
static inline uint64_t mulmod52(uint64_t a,uint64_t b,uint64_t p){
    ...
    uint64_t lo=(uint64_t)z&MASK52;
    uint64_t hi=(uint64_t)(z>>52);
    __uint128_t y=(__uint128_t)lo+(__uint128_t)hi*c;
    uint64_t y0=(uint64_t)y&MASK52;
    uint64_t y1=(uint64_t)(y>>52);
    uint64_t r=y0+y1*c; // < B + small
    if(r>=p)r-=p;
    if(r>=p)r-=p;
    return r;
}
...
struct VMod{
    static inline __m512i mul(__m512i a,__m512i b,__m512i p,__m512i c){
        __m512i z=_mm512_setzero_si512();
        __m512i lo=_mm512_madd52lo_epu64(z,a,b), hi=_mm512_madd52hi_epu64(z,a,b);
        __m512i tlo=_mm512_madd52lo_epu64(z,hi,c), thi=_mm512_madd52hi_epu64(z,hi,c);
        __m512i s=_mm512_add_epi64(lo,tlo), carry=_mm512_srli_epi64(s,52);
        __m512i y0=_mm512_and_si512(s,_mm512_set1_epi64(MASK52)), y1=_mm512_add_epi64(thi,carry);
        __m512i r=_mm512_add_epi64(y0,_mm512_mullo_epi64(y1,c));
        __mmask8 ge=_mm512_cmp_epu64_mask(r,p,_MM_CMPINT_NLT);r=_mm512_mask_sub_epi64(r,ge,r,p);
        return r;
    }
    static inline __m512i mul_small(__m512i a,uint64_t b,__m512i p,__m512i c){
        __m512i z=_mm512_setzero_si512(),vb=_mm512_set1_epi64((long long)b);
        __m512i lo=_mm512_madd52lo_epu64(z,a,vb),hi=_mm512_madd52hi_epu64(z,a,vb);
        __m512i r=_mm512_add_epi64(lo,_mm512_mullo_epi64(hi,c));
        __mmask8 ge=_mm512_cmp_epu64_mask(r,p,_MM_CMPINT_NLT);r=_mm512_mask_sub_epi64(r,ge,r,p);
        return r;
    }
    static inline __m512i add(__m512i a,__m512i b,__m512i p){__m512i s=_mm512_add_epi64(a,b);__mmask8 ge=_mm512_cmp_epu64_mask(s,p,_MM_CMPINT_NLT);return _mm512_mask_sub_epi64(s,ge,s,p);}
    static inline __m512i sub(__m512i a,__m512i b,__m512i p){__mmask8 lt=_mm512_cmp_epu64_mask(a,b,_MM_CMPINT_LT);__m512i d=_mm512_sub_epi64(a,b);return _mm512_mask_add_epi64(d,lt,d,p);}
};
static inline __m512i vsmall(uint64_t x){return _mm512_set1_epi64((long long)x);}
...
struct PackConst{alignas(64) uint64_t p[8],c[8];__m512i vp,vc;std::vector<__m512i> inv,bcommon;};
struct PackState{std::array<__m512i,MAXM> H{},B{};__m512i A=vsmall(1),O=_mm512_setzero_si512(),rankR=_mm512_setzero_si512();};
struct RnsEngine{
...
#pragma omp parallel for schedule(static)
        ...
        for(int pk=0;pk<npacks;pk++){alignas(64)uint64_t rr[8];_mm512_store_si512(rr,initPacks[pk].H[0]);for(int l=0;l<8;l++){int i=pk*8+l;if(i<npr)Nres[i]=rr[l];}}
    }
    void build_pack(int pk){
        auto &q=pc[pk];for(int l=0;l<8;l++){int i=pk*8+l;uint64_t p=i<npr?primes[i]:primes.back();q.p[l]=p;q.c[l]=B52-p;}q.vp=_mm512_load_si512(q.p);q.vc=_mm512_load_si512(q.c);
        int maxv=total+2;q.inv.resize(maxv);std::vector<__m512i> fact(maxv);fact[0]=vsmall(1);for(int i=1;i<maxv;i++)fact[i]=VMod::mul_small(fact[i-1],i,q.vp,q.vc);alignas(64)uint64_t ff[8],ii[8];_mm512_store_si512(ff,fact[maxv-1]);for(int l=0;l<8;l++)ii[l]=inv_mod_u64(ff[l],q.p[l]);__m512i acc=_mm512_load_si512(ii);for(int i=maxv-1;i>=1;i--){q.inv[i]=VMod::mul(acc,fact[i-1],q.vp,q.vc);acc=VMod::mul_small(acc,i,q.vp,q.vc);}q.inv[0]=_mm512_setzero_si512();fact.clear();fact.shrink_to_fit();
        auto &st=initPacks[pk];st.H[m-1]=vsmall(1);st.B[m-1]=_mm512_setzero_si512();
        ...
        std::array<std::vector<__m512i>,124> wc;
        ...
        std::vector<__m512i> following(pref.back()+2,vsmall(1)),current;
        for(int h=m-2;h>=0;--h){int qmax=pref[h]+1,n=len[h+1];current.assign(qmax+1,_mm512_setzero_si512());__m512i cum=_mm512_setzero_si512(),w=vsmall(1),Blast=vsmall(1);bool cached=freq[n]>1;for(int qq=1;qq<=qmax;qq++){int r=qq-1;__m512i ww=cached?wc[n][r]:w;auto term=VMod::mul(ww,following[n+r],q.vp,q.vc);cum=VMod::add(cum,term,q.vp);current[qq]=cum;if(qq==qmax)Blast=ww;if(!cached){w=VMod::mul_small(w,n+r-1,q.vp,q.vc);w=VMod::mul(w,q.inv[qq],q.vp,q.vc);}}st.H[h]=current[qmax];st.B[h]=Blast;following.swap(current);}
        q.bcommon=std::move(wc[cn]);
        st.A=vsmall(1);st.O=_mm512_setzero_si512();st.rankR=_mm512_setzero_si512();
    }
...
static FracAdaptive frac_adaptive(const RnsEngine&e,const std::vector<uint64_t>&res,const std::vector<uint64_t>&coeff,int k,int maxlayers=96){
    ...
    for(int layer=0;layer<maxlayers;layer++){uint64_t sumq=0;long double sum=0,corr=0;for(int i=0;i<k;i++){uint64_t p=e.primes[i],cc=B52-p;__uint128_t cn=(__uint128_t)cc*n[i];uint64_t q0=(uint64_t)(cn>>52),r0=(uint64_t)cn&MASK52;uint64_t y=r0+q0*cc;uint64_t ex=q0;if(y>=p){y-=p;ex++;}sumq+=n[i]+ex;n[i]=y;long double term=(long double)y*e.invp[i];long double yy=term-corr,tt=sum+yy;corr=(tt-sum)-yy;sum=tt;}uint64_t carry=(uint64_t)floorl(sum+1e-18L);uint64_t dig=(sumq+carry)&MASK52;out.layers=layer+1;if(dig){seen=true;sig++;}else if(seen)sig++;val += scalbnl((long double)dig,-52*(layer+1));if(seen&&sig>=3){out.value=val;return out;}}
    out.value=val;if(!seen)out.ok=false;return out;
}
static void gather_vals(const RnsEngine&e,const FastState&s,std::vector<uint64_t>&O,std::vector<uint64_t>&S,std::vector<uint64_t>*U=nullptr,std::vector<uint64_t>*V=nullptr){int k=s.k;O.resize(k);S.resize(k);if(U)U->resize(k);if(V)V->resize(k);int packs=(k+7)/8;for(int pk=0;pk<packs;pk++){const auto&q=e.pc[pk];const auto&z=s.pack[pk];__m512i sv=VMod::mul(z.A,z.H[s.st.high],q.vp,q.vc);__m512i xv=VMod::sub(z.rankR,z.O,q.vp);alignas(64)uint64_t oo[8],ss[8],xx[8];_mm512_store_si512(oo,z.O);_mm512_store_si512(ss,sv);_mm512_store_si512(xx,xv);for(int l=0;l<8;l++){int i=pk*8+l;if(i>=k)break;O[i]=oo[l];S[i]=ss[l];if(U){(*U)[i]=xx[l]==0?e.primes[i]-1:xx[l]-1;}if(V){(*V)[i]=submod52(ss[l],xx[l],e.primes[i]);}}}}

struct FracTriple{FracAdaptive u,v,s;};
static inline __m512i frac_digit_step(__m512i n,const PackConst&q,__m512i &qsumv){
    __m512i z=_mm512_setzero_si512();
    __m512i lo=_mm512_madd52lo_epu64(z,n,q.vc),hi=_mm512_madd52hi_epu64(z,n,q.vc);
    __m512i y=_mm512_add_epi64(lo,_mm512_mullo_epi64(hi,q.vc));
    __mmask8 ge=_mm512_cmp_epu64_mask(y,q.vp,_MM_CMPINT_NLT);
    y=_mm512_mask_sub_epi64(y,ge,y,q.vp);
    __m512i ex=_mm512_mask_add_epi64(hi,ge,hi,vsmall(1));
    qsumv=_mm512_add_epi64(n,ex);
    return y;
}
static FracTriple frac_state_vector(const RnsEngine&e,const FastState&s,const std::vector<uint64_t>&cf,int maxlayers=12){
    FracTriple out;int k=s.k,packs=(k+7)/8;std::vector<__m512i> nu(packs),nv(packs),ns(packs);
    for(int pk=0;pk<packs;pk++){
        const auto&q=e.pc[pk];const auto&z=s.pack[pk];__m512i sv=VMod::mul(z.A,z.H[s.st.high],q.vp,q.vc);__m512i xv=VMod::sub(z.rankR,z.O,q.vp);
        __m512i uv=VMod::sub(xv,vsmall(1),q.vp),vv=VMod::sub(sv,xv,q.vp);
        int base=pk*8,avail=std::min(8,k-base);__mmask8 mask=(__mmask8)((1u<<avail)-1u);__m512i cv=_mm512_maskz_loadu_epi64(mask,cf.data()+base);
    ...
    for(int layer=0;layer<maxlayers;layer++){
        ...
        for(int pk=0;pk<packs;pk++){
            const auto&q=e.pc[pk];__m512i qu,qv,qs;nu[pk]=frac_digit_step(nu[pk],q,qu);nv[pk]=frac_digit_step(nv[pk],q,qv);ns[pk]=frac_digit_step(ns[pk],q,qs);
            int base=pk*8,avail=std::min(8,k-base);alignas(64)uint64_t yu[8],yv[8],ys[8],xu[8],xv[8],xs[8];
            _mm512_store_si512(yu,nu[pk]);_mm512_store_si512(yv,nv[pk]);_mm512_store_si512(ys,ns[pk]);_mm512_store_si512(xu,qu);_mm512_store_si512(xv,qv);_mm512_store_si512(xs,qs);
        ...
        for(int a=0;a<3;a++){if(done[a])continue;uint64_t carry=(uint64_t)floorl(sum[a]+1e-18L);uint64_t dig=(sumq[a]+carry)&MASK52;rr[a]->layers=layer+1;if(dig){if(!seen[a])rr[a]->firstnz=layer+1;seen[a]=true;sig[a]++;}else if(seen[a])sig[a]++;val[a]+=scalbnl((long double)dig,-52*(layer+1));if(seen[a]&&sig[a]>=3)done[a]=true;if(!done[a])all=false;}
...
static void set_rank_resid(const RnsEngine&e,FastState&s){int packs=(s.k+7)/8;for(int pk=0;pk<packs;pk++){alignas(64)uint64_t rr[8]{};for(int l=0;l<8;l++){int i=pk*8+l;if(i<e.npr)rr[l]=mpz_fdiv_ui(s.rank.get_mpz_t(),e.primes[i]);}s.pack[pk].rankR=_mm512_load_si512(rr);s.pack[pk].O=_mm512_setzero_si512();}}

static inline __m512i replay_B(const RnsEngine&e,const PackConst&q,const PackState&z,int h,int d){if(h>=e.m-1)return _mm512_setzero_si512();int Rh=e.pref[h]-d;if(Rh<0)return _mm512_setzero_si512();if(e.commonFreq>1&&e.len[h+1]==e.commonN)return q.bcommon[Rh];return z.B[h];}
static inline __m512i ratio_prod4(const PackConst&q,const StepMeta&x,int from,int to){ // inclusive j range
    __m512i a0=vsmall(1),a1=vsmall(1),a2=vsmall(1),a3=vsmall(1);int z=0;
