...
struct YBMonthDP{
    ...
    YBMonthDP(int T,int P):total(T),parts(P),f(P+1){f[0].resize(1);f[0][0].w[0]=1;for(int p=1;p<=P;p++){int maxShift=119*p,half=maxShift/2;f[p].resize(half+1);YBU320 win{};auto prev=[&](int sh)->const YBU320&{static const YBU320 Z{};int pm=119*(p-1);if(sh<0||sh>pm)return Z;int q=std::min(sh,pm-sh);return f[p-1][q];};for(int sh=0;sh<=half;sh++){yb_uadd(win,prev(sh));if(sh>=120)yb_usub(win,prev(sh-120));f[p][sh]=win;}}}
    ...
    std::vector<int>unrank(BI rankBI)const{YBU320 rank=yb_bi_to_u320(rankBI);int rem=total;std::vector<int>out;out.reserve(parts);for(int pos=0;pos<parts;pos++){int left=parts-pos-1,maxv=std::min(123,rem-4*left);bool sel=false;for(int v=4;v<=maxv;v++){int after=rem-v;const YBU320&block=at(left,after);if(yb_ucmp(rank,block)>0){yb_usub(rank,block);continue;}out.push_back(v);rem=after;sel=true;break;}if(!sel)throw std::runtime_error("month rank exhausted");}return out;}
};
