...
struct Gates{
    std::vector<uint16_t> gap;std::vector<int64_t> pos;
    explicit Gates(const std::string&fn){std::ifstream f(fn,std::ios::binary);if(!f)throw std::runtime_error("cannot open gates");f.seekg(0,std::ios::end);size_t bytes=(size_t)f.tellg();f.seekg(0);if(bytes%2)throw std::runtime_error("odd gate file");gap.resize(bytes/2);for(size_t i=0;i<gap.size();i++){unsigned char b[2];f.read((char*)b,2);gap[i]=(uint16_t)b[0]|((uint16_t)b[1]<<8);}pos.resize(gap.size()+1);pos[0]=reference::FOUNDATION;for(size_t i=0;i<gap.size();i++)pos[i+1]=pos[i]+gap[i];}
    int contain(int64_t d)const{auto it=std::lower_bound(pos.begin()+1,pos.end(),d);if(it==pos.end())throw std::runtime_error("day beyond positive gate corpus");return int(it-pos.begin())-1;}
    int64_t at(int i)const{return pos.at(i);}
};
struct Year{long long num;int o,c;int64_t a,b;};
struct Candidate{int o,c;int64_t len;};
static Year anchor_year(int64_t calc,const Gates&G,bool saved){int k=G.contain(calc);std::vector<Candidate>v;for(int o=k;o>=0&&calc-G.at(o)<=5778;o--)for(int c=k+1;c<(int)G.pos.size()&&G.at(c)-calc<=5778;c++){int64_t len=G.at(c)-G.at(o);if(c-o>=6&&len>=252&&len<=5778)v.push_back({o,c,len});}std::sort(v.begin(),v.end(),[](auto&a,auto&b){return a.len!=b.len?a.len<b.len:a.o<b.o;});auto so=reference::sauce(calc,calc,saved);BI rank=choose_ref(so,1,10,BI(v.size()));auto q=v[(rank-1).convert_to<size_t>()];return{5000,q.o,q.c,G.at(q.o),G.at(q.c)};}
static Year adjacent_year(int64_t calc,const Gates&G,const Year&y,bool next,bool saved){int fixed=next?y.c:y.o;if(next){int first=fixed+6,last=first-1;for(int c=first;c<(int)G.pos.size();c++){if(G.at(c)-G.at(fixed)>5778)break;last=c;}int n=last-first+1;if(n<=0)throw std::runtime_error("no next year in corpus");auto so=reference::sauce(calc,G.at(fixed),saved);int rank=choose_ref(so,1,11,BI(n)).convert_to<int>();int c=first+rank-1;return{y.num+1,fixed,c,G.at(fixed),G.at(c)};}int first=fixed-6;if(first<0)throw std::runtime_error("no previous year in positive corpus");int last=first+1;for(int o=first;o>=0;o--){if(G.at(fixed)-G.at(o)>5778)break;last=o;}int n=first-last+1;if(n<=0)throw std::runtime_error("no previous year");auto so=reference::sauce(calc,G.at(fixed),saved);int rank=choose_ref(so,1,12,BI(n)).convert_to<int>();int o=first-rank+1;return{y.num-1,o,fixed,G.at(o),G.at(fixed)};}
static Year find_year(int64_t calc,int64_t target,const Gates&G,bool saved,int&steps){Year y=anchor_year(calc,G,saved);steps=0;while(target<y.a+1){y=adjacent_year(calc,G,y,false,saved);steps++;}while(target>y.b){y=adjacent_year(calc,G,y,true,saved);steps++;}return y;}
...
struct Result{Year y;int steps,offset,cutletIndex,dayCutlet,monthIndex,dayMonth,monthCount,weaveWidth;unsigned weaveBits;};
static Result calculate(int64_t calc,int64_t target,const Gates&G,bool saved){int steps=0;Year y=find_year(calc,target,G,saved,steps);int yearLen=(int)(y.b-y.a),gapCount=y.c-y.o,offset=(int)(target-(y.a+1));auto so=reference::sauce(calc,y.a+1,saved);
 ...
 return{y,steps,offset,cutNames[cut],dayCut,monthNames[month],dayMonth,monthCount,width,bits};}

int main(int argc,char**argv){if(argc<4){std::cerr<<"usage: canonical_vector_oracle GATES calc target [--raw-mutant]\n";return 2;}bool saved=!(argc>4&&std::string(argv[4])=="--raw-mutant");Gates G(argv[1]);int64_t calc=std::stoll(argv[2]),target=std::stoll(argv[3]);auto r=calculate(calc,target,G,saved);std::cout<<"year="<<r.y.num<<" steps="<<r.steps<<" gates="<<r.y.o<<":"<<r.y.c<<" len="<<(r.y.b-r.y.a)<<" offset="<<r.offset<<" cutlet_idx="<<r.cutletIndex<<" day_cutlet="<<r.dayCutlet<<" month_idx="<<r.monthIndex<<" day_month="<<r.dayMonth<<" months="<<r.monthCount<<" Nbits="<<r.weaveBits<<" width="<<r.weaveWidth<<"\n";}
