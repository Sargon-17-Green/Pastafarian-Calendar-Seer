...
Exact work is admission-controlled. `SEER_EXACT_CONCURRENCY` sets the process-wide native concurrency cap and defaults to 8; `SEER_EXACT_QUEUE_MAX` bounds queued exact work inside the exact provider/engine and defaults to 256. The precomputed provider uses the same concurrency default when scheduling independent calculation-day groups, so contiguous runs can still be coalesced while unrelated groups are bounded. `SEER_SERVICE_QUEUE_MAX` separately bounds pending requests on the persistent service and defaults to 64. These native/provider limits do **not** by themselves bound HTTP clients waiting above provider batching/coalescing.

For public hosted HTTP service admission, set `SEER_HTTP_EXACT_CONCURRENCY` and optionally `SEER_HTTP_EXACT_QUEUE_MAX`. When `SEER_HTTP_EXACT_CONCURRENCY` is unset, the HTTP layer preserves the legacy behavior and adds no separate admission cap. When it is set, `SEER_HTTP_EXACT_QUEUE_MAX` defaults to 0 if omitted. Heavy exact endpoints (`/v1/now`, `/v1/date`, `/v1/batch`, `/v1/range`, `/v1/reverse`, and `/v1/year/{year}`) pass through this gate; liveness, readiness/status, metadata, locales, static/API-schema delivery, and calculation-day resolution remain outside it. HTTP admission overflow is returned as typed HTTP 503 `SEER_UNAVAILABLE` with `details.admissionFailure="overloaded"` and `details.layer="http"`. Node callers constructing the HTTP server may equivalently use `maxHttpExactConcurrency` and `maxHttpExactQueue`.

Queue saturation at the native/provider layers is likewise reported as typed `SEER_UNAVAILABLE` overload instead of growing memory or spawning an unbounded fallback fan-out. Node callers may use `maxExactConcurrency` and `maxExactQueue` to apply a stricter provider/engine-local bound; the process-wide native cap remains controlled by the environment.
