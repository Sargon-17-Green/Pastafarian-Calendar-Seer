...
#include "year_fast_bench_v3_reference.cpp"
#undef main
...
static int conservative_npr(const std::vector<int>&len){
    int total=std::accumulate(len.begin(),len.end(),0);long double ln=lgammal((long double)total+1);for(int x:len)ln-=lgammal((long double)x+1);long double bits=ln/logl(2.0L);int k=(int)ceill((bits+32.0L)/31.9L)+2;return std::clamp(k,8,1050);
}

int main(int argc,char**argv){
    if(argc<3){std::cerr<<"calc target [threads] [sb]\n";return 2;}int64_t calc=std::stoll(argv[1]),target=std::stoll(argv[2]);int threads=argc>3?atoi(argv[3]):3,sb=argc>4?atoi(argv[4]):512,replayThreads=argc>5?atoi(argv[5]):threads;
    ...
    std::cout<<"year="<<y.num<<" steps="<<steps<<" gates="<<y.o<<":"<<y.c<<" len="<<(y.b-y.a)<<" offset="<<offset
             ...
             <<" months="<<st.monthCount<<" npr="<<npr<<" count_threads="<<threads<<" replay_threads="<<replayThreads<<" Nbits="<<mpz_sizeinbase(N.get_mpz_t(),2)<<" width="<<width<<"\n";
