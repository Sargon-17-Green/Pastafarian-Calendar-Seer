...
static inline __m256i reduce32x4_small(__m256i z,__m256i p64,__m256i c64){
    // Exact for scalar multipliers used by Seer: b <= total <= 5778 and c <= 23177.
    // Then y=lo+hi*c < 2^33, so the second Mersenne-style fold has a 0/1 carry.
    const __m256i mask=_mm256_set1_epi64x(0xffffffffULL);
    __m256i lo=_mm256_and_si256(z,mask),hi=_mm256_srli_epi64(z,32);
    __m256i y=_mm256_add_epi64(lo,_mm256_mul_epu32(hi,c64));
    __m256i carry=_mm256_srli_epi64(y,32);
    __m256i carryMask=_mm256_sub_epi64(_mm256_setzero_si256(),carry); // 0 or -1
    __m256i r=_mm256_add_epi64(_mm256_and_si256(y,mask),_mm256_and_si256(carryMask,c64));
    __m256i gt=_mm256_cmpgt_epi64(r,p64),eq=_mm256_cmpeq_epi64(r,p64),ge=_mm256_or_si256(gt,eq);
    return _mm256_sub_epi64(r,_mm256_and_si256(ge,p64));
}
static inline __m256i mulsmall32x8(__m256i a,uint32_t b,__m256i p,__m256i c){
    const __m256i mask=_mm256_set1_epi64x(0xffffffffULL);
    __m256i pe=_mm256_and_si256(p,mask),po=_mm256_srli_epi64(p,32);
    __m256i ce=_mm256_and_si256(c,mask),co=_mm256_srli_epi64(c,32);
    __m256i bv=_mm256_set1_epi32((int)b);
    __m256i ze=_mm256_mul_epu32(a,bv);
    __m256i zo=_mm256_mul_epu32(_mm256_srli_epi64(a,32),bv);
    __m256i re=reduce32x4_small(ze,pe,ce),ro=reduce32x4_small(zo,po,co);
    return _mm256_or_si256(re,_mm256_slli_epi64(ro,32));
}
...
struct VMod{
    ...
    static inline V8 mul_small(const V8&a,uint64_t b,const V8&p,const V8&c){assert(b<=5778);return {mulsmall32x8(a.v,(uint32_t)b,p.v,c.v)};}
