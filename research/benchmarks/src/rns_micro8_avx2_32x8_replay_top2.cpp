...
struct PackConst{alignas(64) uint64_t p[8],c[8];V8 vp,vc;std::vector<V8> inv;std::array<int,124> bslot{};std::vector<std::vector<V8>> brepeat;};
...
#pragma omp parallel for schedule(static)
    ...
    void build_pack(int pk){
        ...
        q.bslot.fill(-1);q.brepeat.clear();
        std::array<int,123> cand{};int cc=0;for(int n=1;n<=123;n++)if(freq[n]>1)cand[cc++]=n;
        std::sort(cand.begin(),cand.begin()+cc,[&](int a,int b){if(freq[a]!=freq[b])return freq[a]>freq[b];return maxrByN[a]>maxrByN[b];});
        int keep=std::min(2,cc);q.brepeat.reserve(keep);for(int i=0;i<keep;i++){int n=cand[i];q.bslot[n]=(int)q.brepeat.size();q.brepeat.push_back(std::move(wc[n]));}
...
static inline V8 replay_B(const RnsEngine&e,const PackConst&q,const PackState&z,int h,int d){if(h>=e.m-1)return vzero();int Rh=e.pref[h]-d;if(Rh<0)return vzero();int n=e.len[h+1],slot=q.bslot[n];if(slot>=0)return q.brepeat[slot][Rh];return z.B[h];}
...
static void replay_pack(const RnsEngine&e,PackState&z,int pk,const std::vector<StepMeta>&steps){
    const auto&q=e.pc[pk];
    for(const auto&x:steps){
        V8 activeBh=vzero(),activeH;
        if(x.high>=e.m-1){activeH=z.H[x.high];}
        else{activeBh=replay_B(e,q,z,x.high,x.d);auto ot=VMod::mul(activeBh,z.H[x.high+1],q.vp,q.vc);activeH=VMod::sub(z.H[x.high],ot,q.vp);}
        if(x.open){
            auto before=VMod::mul(z.A,activeH,q.vp,q.vc);z.O=VMod::add(z.O,before,q.vp);z.A=VMod::mul(z.A,activeBh,q.vp,q.vc);
        }else{
            V8 Ck=z.A,Cprev=vzero();int k=x.chosen;
            if(k<x.high){auto rat=ratio_prod4(q,x,k,x.high-1);Ck=VMod::mul(z.A,rat,q.vp,q.vc);}
            if(k>x.low){int j=k-1;uint64_t num=x.cum[j],den=(uint64_t)x.cum[j+1]-1;auto f=VMod::mul_small(q.inv[den],num,q.vp,q.vc);Cprev=VMod::mul(Ck,f,q.vp,q.vc);}
            auto before=VMod::mul(Cprev,activeH,q.vp,q.vc);z.O=VMod::add(z.O,before,q.vp);z.A=VMod::sub(Ck,Cprev,q.vp);
            for(int h=x.high;h<e.m-1;h++){
                auto bh=(h==x.high)?activeBh:replay_B(e,q,z,h,x.d);
                auto term=VMod::mul(bh,z.H[h+1],q.vp,q.vc);z.H[h]=VMod::sub(z.H[h],term,q.vp);
                int Rh=e.pref[h]-x.d,n=e.len[h+1];
                if(q.bslot[n]>=0)continue; // cached rows are looked up directly; z.B[h] is dead for them.
                if(Rh<=0)z.B[h]=vzero();
                else{z.B[h]=VMod::mul_small(z.B[h],Rh,q.vp,q.vc);z.B[h]=VMod::mul(z.B[h],q.inv[Rh+n-2],q.vp,q.vc);}
            }
        }
    }
}
