...
#pragma omp parallel for schedule(static)
    ...
    void build_pack(int pk){
        ...
        q.bslot.fill(-1);q.brepeat.clear();q.brepeat.reserve(16);for(int n=1;n<=123;n++)if(freq[n]>1){q.bslot[n]=(int)q.brepeat.size();q.brepeat.push_back(std::move(wc[n]));}
    ...
    for(const auto&x:steps){
        ...
        if(x.open){
            ...
            for(int h=x.high;h<e.m-1;h++){
                ...
                if(q.bslot[n]>=0)continue; // replay_B reads the retained repeated-length table directly; z.B[h] is dead for this row.
                if(Rh<=0)z.B[h]=vzero();
