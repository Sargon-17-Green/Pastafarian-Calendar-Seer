...
The request instant is captured once by the HTTP adapter and passed to the shared query layer. CORS is enabled for public read/query use without credentials. Infrastructure health is separate: /_health/live performs no engine/cache work, while /_health/ready is a bounded non-semantic exact-runtime/cache-state probe. /v1/status exposes the same coarse public-safe state without internal details.
