...
The current repository container healthcheck calls public `/v1/status`, and the current status implementation calls `queryDate()`. Before public launch, the hosting/observability workstream must either separate provider health probes from the public route or prove and enforce a bounded cheap readiness implementation. A high-frequency healthcheck must not repeatedly trigger expensive exact work.
