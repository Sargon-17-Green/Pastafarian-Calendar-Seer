...
`v0.2.4` is intentionally recorded as a **partial release** at commit `1108622a527af4ad55b4a3f964120471eba8a4d0`. Its unified release-candidate verification passed, and the three native npm runtime packages `0.2.4` plus the GHCR multi-architecture image were published and verified from that tag. The root npm workflow first exhausted its 30-minute wait for the native publication while the Windows native job was still running; after the native workflow later completed successfully, a safe retry exposed a second workflow defect in `Verify npm package boundary`: the step referenced `$VERSION` under `set -u` without binding it. The root npm package and final GitHub Release were therefore not published. The tag and already-published native/GHCR artifacts are not moved, overwritten, or backfilled under corrected workflow code; recovery proceeds as `v0.2.5`.
