...
Remediation was implemented in PR #37,
...
Live re-validation subsequently passed, and PR #37 was merged after a fresh
post-PR-#36 rebase and fully green CI.
...
zero-payment launch provider. All Render live-provider gates recorded above have passed.

## Integrated hosting dependencies — 2026-09-21

The live-provider evidence above intentionally retains the exact commits that were
actually deployed during evaluation. The hosting implementation dependencies are
now integrated into `main`:

- PR #36 review head `3155cab3702a291bdccffb6bfee07ed02602a1e4`
  merged as `9ff096b66c5ecfbd214d6e6b8747e79a66f7ca2c`;
- PR #37 was rebased onto that `main` state as
  `2817cc130cb082dc65db6e457bcdaeb832ba0b9c` and merged as
  `93000cbdca5b2d2666ea716717e342e9a5c4f0e3`.

Thus the implementation prerequisites are no longer pending review dependencies.

### Live-to-integrated runtime equivalence

The overload remediation was exercised live on Render at
`69cc3191d197885ef9b5b508387cb847edf4fdba`, while the final PR #37 integration
head is `2817cc130cb082dc65db6e457bcdaeb832ba0b9c`.

The runtime-bearing files relevant to that proof are byte-identical between the
two commits: `http/app.mjs`, `http/server.mjs`, `query/exact-engine.mjs`,
`query/provider-precomputed.mjs`, and the `Dockerfile`. The only
`package.json` differences are the root version and the three optional native
package versions moving from 0.2.3 to 0.2.5.

The container explicitly builds the local native runtime, and exact runtime
resolution prefers those local `prototype/build` binaries before optional
prebuilt packages. Therefore the final v0.2.5 integration does not change the container runtime
path exercised by the live overload proof. A repeat live behavior gate is not
required solely because of the merge ancestry change.
